# Capability: Proto Message Doc View

## Purpose

Document the data structure of a `message` in a `.proto` file. The message under the caret is parsed via Protocol Buffers plugin PSI, synthesized into an in-memory Java class (nested and imported types, `repeated`, `map`, `oneof`, enums and `google.protobuf` well-known types included), and rendered through the existing POJO document pipeline as a Doc View preview. Deliberately excludes `service` / `rpc` interface documentation, which `grpc-copy-curl` covers.

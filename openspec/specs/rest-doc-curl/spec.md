# Capability: REST Doc cURL

## Purpose

Build a curl command for every Spring or Feign REST endpoint from the data already on `DocView`, expose it to Velocity templates as a fenced Markdown block, and carry it through every Markdown output path.
## Requirements
